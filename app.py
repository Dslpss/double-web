...
pragmatic_analyzer = None  # Nova integração Pragmatic Play
    ...
    try:
        ...
        if not pragmatic_analyzer_available:
            return jsonify({
                ...
                'connected': False,
                'monitoring': False,
                'error': 'PragmaticAnalyzer não está disponível'
            })
        ...
        if pragmatic_analyzer is None:
            return jsonify({
                ...
                'connected': False,
                'monitoring': False,
                'message': 'PragmaticAnalyzer não inicializado'
            })
...
def pragmatic_start():
    """Iniciar monitoramento PragmaticAnalyzer."""
    try:
        global pragmatic_analyzer
        ...
        if not username or not password:
            return jsonify({
                'success': False,
                'error': 'Credenciais PlayNabets não configuradas'
            }), 400
        
        if pragmatic_analyzer is None:
            pragmatic_analyzer = initialize_pragmatic_analyzer(username, password)
        
        # Iniciar monitoramento em thread separada
        def start_monitoring():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(pragmatic_analyzer.start_monitoring())
        ...
        return jsonify({
            'success': True,
            'message': 'Monitoramento PragmaticAnalyzer iniciado'
        })
...
def pragmatic_stop():
    """Parar monitoramento PragmaticAnalyzer."""
    try:
        global pragmatic_analyzer
        ...
        if pragmatic_analyzer is None:
            return jsonify({
                'success': False,
                'error': 'PragmaticAnalyzer não inicializado'
            }), 400
        
        # Parar monitoramento em thread separada
        def stop_monitoring():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(pragmatic_analyzer.stop_monitoring())
        
        thread = threading.Thread(target=stop_monitoring)
        thread.daemon = True
        thread.start()
        
        return jsonify({
            'success': True,
            'message': 'Monitoramento PragmaticAnalyzer parado'
        })
...
@app.route('/api/pragmatic/results')
def pragmatic_results():
    """Obter resultados do PragmaticAnalyzer."""
    try:
        global pragmatic_analyzer
        ...
        results = pragmatic_analyzer.get_results(limit)
