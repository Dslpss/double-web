...
"""
Monitor direto do jogo Brazilian Roulette
URL: https://qcxjeqo01e.cjlcqchead.net/gs2c/playGame.do
"""
...
class DirectGameMonitor:
    """Monitor direto do jogo."""
    
    def __init__(self):
        self.game_url = "https://qcxjeqo01e.cjlcqchead.net/gs2c/playGame.do?key=token%3DeyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.WzYzMzE0XQ.1BxRGal62pouh2TU9gbeCsIjOfPPUP1WqQY5ZunViso%60%7C%60symbol%3D237%60%7C%60language%3Dpt%60%7C%60lobbyUrl%3Dhttps%3A%2F%2Fplaynabet.com%2Fcasino&ppkv=2&stylename=weebet_playnabet&isGameUrlApiCalled=true"
        self.running = False
    ...
    async def monitor_game(self):
        """Monitora o jogo diretamente."""
        try:
            print(f"🔍 Monitorando jogo: Brazilian Roulette")
            ...
            async with aiohttp.ClientSession(headers=self.get_headers()) as session:
                while self.running:
                    try:
                        ...
                        async with session.get(self.game_url, verify_ssl=False) as response:
    ...
    def start(self):
        ...
        import threading
        self.thread = threading.Thread(target=run_async, name="DirectGameMonitor")
        self.thread.start()
...
async def main():
    """Função principal."""
    print("🎯 Monitor Direto do Jogo - Brazilian Roulette")
    print("=" * 60)
    print("Acessando diretamente o jogo Pragmatic Play")
    print("Pressione Ctrl+C para parar")
    print()
    
    monitor = DirectGameMonitor()
